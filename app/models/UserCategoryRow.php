...
class UserCategoryRow {
    public static function getByUser(int $userId, string $mode): array {
        $st = db()->prepare("
            ...
            WHERE ucr.user_id = ? AND ucr.mode = ?
            ORDER BY t.sort, c.name
        ");
        $st->execute([$userId, $mode]);
        return $st->fetchAll();
    ...
    public static function findOrCreate(int $userId, string $mode, int $tabId, int $categoryId): array {
        ...
        $st = $dbh->prepare("
            SELECT * FROM user_category_rows 
            WHERE user_id = ? AND tab_id = ? AND category_id = ? AND mode = ?
        ");
        $st->execute([$userId, $tabId, $categoryId, $mode]);
        $existing = $st->fetch();
        ...
        // Create new
        $st = $dbh->prepare("
            INSERT INTO user_category_rows (user_id, tab_id, category_id, mode, current_total_cents)
            VALUES (?, ?, ?, ?, 0)
        ");
        $st->execute([$userId, $tabId, $categoryId, $mode]);
    ...
    public static function addEntry(int $rowId, int $userId, int $cents, string $currency, string $memo, string $idempotencyKey): void {
        $dbh = db();
        $dbh->beginTransaction();
        ...
        try {
            // Add entry
            $st = $dbh->prepare("
                INSERT INTO entries (row_id, user_id, amount_cents, currency, memo, idempotency_key)
                VALUES (?, ?, ?, ?, ?, ?)
            ");
            $st->execute([$rowId, $userId, $cents, $currency, $memo, $idempotencyKey]);
