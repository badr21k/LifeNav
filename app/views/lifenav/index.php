...
<div class="container mt-3">
  <div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <div class="d-flex align-items-center gap-2">
      ...
      <a href="/lifenav/reports/monthly?mode=<?= htmlspecialchars($mode) ?>" class="btn btn-outline-info btn-sm">Reports</a>
    </div>
  </div>
  ...
  <?php foreach ($tabs as $tab): ?>
    <div class="card mb-4">
      <div class="card-header"><h5 class="mb-0"><?= htmlspecialchars($tab['name']) ?></h5></div>
      <div class="card-body">
        <?php
          // pull categories for this tab from *your* table
          $st = db()->prepare("SELECT * FROM app_categories WHERE tab_id=? AND is_active=1 AND (user_id=0 OR user_id=?) ORDER BY is_custom, name");
          $st->execute([$tab['id'], $userId]);
          $categories = $st->fetchAll();

          // rows for this tab keyed by category_id (provided by controller)
        ...
        <div class="row g-2">
          <?php foreach ($categories as $category): ?>
            <?php
              ...
              $userRow = $userRowsForTab[$catId] ?? null;
              $isActive = (bool)$userRow;
            ?>
            <?php if ($isActive): ?>
              <?php $total = number_format($userRow['current_total_cents'] / 100, 2); ?>
              <div class="col-auto">
                <a href="/lifenav/category/<?= (int)$userRow['id'] ?>" class="btn btn-success position-relative">
            ...
            <?php else: ?>
              <div class="col-auto">
                <form method="post" action="/lifenav/select_category" class="d-inline">
                  ...
                  <input type="hidden" name="tab_id" value="<?= (int)$tab['id'] ?>">
          ...
          <?php if (empty($categories)): ?>
            <div class="col-12 text-muted">No categories in this tab.</div>
          <?php endif; ?>
        </div>
      </div>
    </div>
